...
def build_features(dataframe: pd.DataFrame, mlb: MultiLabelBinarizer | None = None) -> Tuple[pd.DataFrame, MultiLabelBinarizer]:
    ...
    age_values = df["Age"].astype(int)
    pediatric_mask = age_values < 18
    bp_s_baseline = np.where(pediatric_mask, 105, 120)
    bp_d_baseline = np.where(pediatric_mask, 65, 80)
    hr_baseline = np.where(pediatric_mask, 110, 70)
    bp_s_scale = np.where(pediatric_mask, 15, 20)
    bp_d_scale = np.where(pediatric_mask, 10, 10)
    hr_scale = np.where(pediatric_mask, 35, 30)

    # Pediatric cases use age-appropriate baselines to avoid adult-threshold bias.
    df["bp_risk_score"] = ((df["bp_systolic"] - bp_s_baseline) / bp_s_scale) + ((df["bp_diastolic"] - bp_d_baseline) / bp_d_scale)
    df["hr_risk_score"] = (df["Heart Rate"] - hr_baseline).abs() / hr_scale
    df["temp_risk_score"] = (df["Temperature"] - 98.6).abs() / 2
    df["is_pediatric"] = pediatric_mask.astype(int)
...
def train(paths: Paths, seed: int = 42) -> Dict[str, object]:
    ...
    x_ood = feature_df.reindex(columns=all_columns, fill_value=0.0).to_numpy(dtype=float)
    ood_mean = x_ood.mean(axis=0)
    cov = np.cov(x_ood, rowvar=False)
    cov = cov + np.eye(cov.shape[0]) * 1e-6
    inv_cov = np.linalg.pinv(cov)
    deltas = x_ood - ood_mean
    distances = np.sqrt(np.einsum("ij,jk,ik->i", deltas, inv_cov, deltas))
    ood_threshold_99 = float(np.percentile(distances, 99))
    preprocessor_artifact["ood_detector"] = {
        "mean": ood_mean.tolist(),
        "inv_cov": inv_cov.tolist(),
        "threshold_99": ood_threshold_99,
        "feature_count": int(len(all_columns)),
    }
    metrics["ood_threshold_99"] = ood_threshold_99
